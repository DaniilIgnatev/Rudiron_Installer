...
ColumnLayout{
    ...
    function contentQML() {
        return tabs[selectedPos].contentQML
    }
        ...
        if (component !== null){
            if (component.status === Component.Ready){
                for (var i = 0; i < tabContent.children.length;i++){
                    tabContent.children[i].destroy()
                }

                content = component.createObject(tabContent)
                content.width = Qt.binding(function(){return tabContent.width})
                content.height = Qt.binding(function(){return tabContent.height})

                if (qmlStr === "contentUI/StagesObserverUI.qml"){
                    var selectedTab = tabBar.tabs[tabBar.selectedPos]
                    var model = selectedTab.model[0]
                    content.imagesURLList = model.imagesURLList
                    content.hintList = model.hintList
                    content.installFunction = model.installFunction
                }
            }
    ...
    Tab{
        id: tab0
        onCheckedChanged: layout.tabChecked(position)
        position: 0
        text: "Описание"
    ...
    Tab{
        ...
        enabled: installerUI.licenceAccepted
        model: [{
    ...
    Tab{
        ...
        enabled: installerUI.licenceAccepted
        model: [{
                imagesURLList: ["../slides/guide/1.png", "../slides/guide/2.png", "../slides/guide/3.png", "../slides/guide/4.png", "../slides/guide/5.png", "../slides/guide/6.png", "../slides/guide/7.png", "../slides/guide/8.png", "../slides/guide/9.png", "../slides/guide/10.png", "../slides/guide/11.png", "../slides/guide/12.png", "../slides/guide/13.png", "../slides/guide/14.png", "../slides/guide/15.png", "../slides/guide/16.png", "../slides/guide/17.png", "../slides/guide/18.png", "../slides/guide/19.png", "../slides/guide/20.png", "../slides/guide/21.png", "../slides/guide/22.png", "../slides/guide/23.png", "../slides/guide/24.png", "../slides/guide/25.png", "../slides/guide/26.png", "../slides/guide/27.png", "../slides/guide/28.png", "../slides/guide/29.png"],
                hintList: ["В вкладке \"Начало\" выберете \"Настроить\" сессию","Нажмите \"Новая\"","Введите название и нажмите \"Создать\"","Выберите созданную сессию и нажмите \"Активировать\"","Нажмите \"Открыть файл или проект\"","Откройте .qbs файл проекта","Раскройте дерево проектов до \"app.cpp\"","Окно запуска программы","Окно предупреждений и ошибок","Окно поиска в проекте","Окно стандартного потока вывода","Информация о ходе сборки программы","Стек вызовов функций","Точки останова","Слева от порядкового номера строки выставляются точки останова","Окно локальных переменных","Окно регистров","В окне регистров переферии нажмите ПКМ","Список регистров выбранной переферии","Нажмите ПКМ для выбора формата отображения значений","Наведите на регистр для подробного вывода значения","Нажмите ПКМ для точного выставления точки останова","Продолжение отладки программы","Завершение отладки программы","Переключение отладчика в режим дизассемблера","В кладке \"Проекты\" можно изменить каталог сборки","Для настройки QtCreator перейдите в \"Инструменты->Параметры\"","Настройки текстового редактора","Рекомендуется шрифт DejaVu Sans Mono"],
