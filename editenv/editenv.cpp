...
unsigned int envCut (env_scope scope, wchar_t const *name, wchar_t const *text)
{
...
void envPaste (env_scope scope, wchar_t const *name, wchar_t const *text)
{
    EnvVar var(scope, name);
...
void envSet (env_scope scope, wchar_t const *name, wchar_t const *text)
{
    EnvVar var(scope, name);
...
void envUnset (env_scope scope, wchar_t const *name)
{
    EnvVar var(scope, name);
...
wchar_t const * envValue (env_scope scope, wchar_t const *name)
{
    EnvVar var(scope, name);
...
void pathAdd (env_scope scope, wchar_t const *path)
{
...
unsigned int pathRemove (env_scope scope, wchar_t const *path)
{
    size_t const sizeMax = -1;// std::numeric_limits<size_t>::max();
    ...
    while (sizeMax != pos) {
        if (((0 == pos) ||
            ...
            if (0 == pos) {
                ...
                // Remove the preceding semicolon along with the path wstring.
